...
<style>
    ...
    .eco-bus-header h2 span {
        color: var(--accent-color); /* Viking Red Accent */
    }
    ...
    #canvas-container {
        ...
        border-radius: 12px;
        overflow: hidden;
        background: radial-gradient(circle at center, #271313 0%, #0f0d0d 100%); /* Red Glow Radial Background */
    ...
    .canvas-loader {
        ...
        width: 100%;
        height: 100%;
        background: #141111; /* Dark Red tint black */
        display: flex;
        flex-direction: column;
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function spawnBird() {
            ...
            const body = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.45, 4), new THREE.MeshBasicMaterial({ color: 0xeeeeee }));
            body.rotation.x = Math.PI / 2;
            bird.add(body);

            const beak = new THREE.Mesh(new THREE.ConeGeometry(0.04, 0.18, 4), new THREE.MeshBasicMaterial({ color: 0xff9500 }));
            beak.position.set(0, 0, 0.3);
            beak.rotation.x = Math.PI / 2;
            bird.add(beak);

            const wingL = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.02, 0.18), new THREE.MeshBasicMaterial({ color: 0xcccccc }));
        ...
        function triggerFeathersBurst(x, y, z) {
            ...
            const mat = new THREE.PointsMaterial({ color: 0xeeeeee, size: 0.22, transparent: true, opacity: 1.0 });
            const system = new THREE.Points(geom, mat);
            scene.add(system);

            gsap.to(mat, { opacity: 0.0, duration: 1.2, delay: 0.4, onComplete: () => { scene.remove(system); geom.dispose(); mat.dispose(); } });
            activeFeathers.push({ system, geometry: geom, velocities, count: burstFeatherCount });
        }
        ...
        function loadModel() {
            new THREE.GLTFLoader().load('/Bus.glb', (gltf) => {
                ...
                gsap.to(busModel.position, { y: startY, duration: 0.7, ease: 'power2.in', onComplete: () => { isSquashing = false; playLandingAnimation(); } });
            }, undefined, (err) => { console.error(err); showFallback(); });
        }

        function showFallback() { loaderEl.style.display = 'none'; fallbackEl.classList.remove('hidden'); canvas.style.display = 'none'; }
        ...
        function animate() {
            ...
            if (particleSystem) {
                ...
                for (let i = 0; i < particleCount; i++) {
                    ...
                    const zA = pos[i*6+2];
                    if (zA > -1.8 && zA < 1.8) {
                        const xA = pos[i*6], yA = pos[i*6+1];
                        if (Math.abs(xA) < 0.85 && yA < 0.85) {
                            if (yA > 0.35) { pos[i*6+1] = pos[i*6+4] = 0.88; }
                            else { const dX = xA >= 0 ? 0.88 : -0.88; pos[i*6] = pos[i*6+3] = dX; }
                        }
                    }
                    if (pos[i*6+2] < -12.0) {
                        const nX = (Math.random()-0.5)*20, nY = Math.random()*6.5-0.5, nZ = 12, sL = Math.random()*0.4+0.3;
                        pos[i*6]=nX; pos[i*6+1]=nY; pos[i*6+2]=nZ; pos[i*6+3]=nX; pos[i*6+4]=nY; pos[i*6+5]=nZ-sL;
                    }
                }
            ...
            for (let k = activeBirds.length - 1; k >= 0; k--) {
                const b = activeBirds[k]; if (!b.active) continue;
                b.mesh.position.z -= b.speed;
                b.wings[0].rotation.z = Math.sin(animationTime * 2) * 0.6; b.wings[1].rotation.z = -Math.sin(animationTime * 2) * 0.6;
                ...
                if (busModel) {
                    ...
                    if (ints.length > 0) { coll = true; b.mesh.position.copy(ints[0].point); b.mesh.position.z += 0.12; }
                }

                if (coll) {
                    b.active = false; triggerFeathersBurst(b.mesh.position.x, b.mesh.position.y, b.mesh.position.z);
                    if (busModel && !isSquashing) { gsap.timeline().to(busModel.rotation, { z: 0.04, duration: 0.08 }).to(busModel.rotation, { z: 0, duration: 0.5, ease: 'elastic.out' }); }
                    const bMesh = b.mesh;
                    gsap.timeline().to(bMesh.scale, { x: 1.3, y: 1.3, z: 0.2, duration: 0.05 })
                        .to(bMesh.position, { x: bMesh.position.x >= 0 ? 4 : -4, z: bMesh.position.z + 0.35, duration: 1.2, ease: 'power2.in', delay: 0.65 })
                        .to(bMesh.rotation, { z: Math.PI * 2, duration: 1.2, ease: 'power2.in' }, "-=1.2")
                        .to(bMesh.position, { z: -25.0, duration: 0.9, ease: 'power2.in' }, "-=0.7")
                        .call(() => scene.remove(bMesh));
                    activeBirds.splice(k, 1);
                } else if (b.mesh.position.z < -12.0) { scene.remove(b.mesh); activeBirds.splice(k, 1); }
            }

            for (let j = activeFeathers.length - 1; j >= 0; j--) {
                const f = activeFeathers[j]; if (f.system.material.opacity <= 0) { activeFeathers.splice(j, 1); continue; }
                const p = f.geometry.attributes.position.array;
                for (let i = 0; i < f.count; i++) { p[i*3]+=f.velocities[i].x; p[i*3+1]+=f.velocities[i].y; p[i*3+2]+=f.velocities[i].z; f.velocities[i].y-=0.0025; }
        ...
        function playLandingAnimation() {
            if (!busModel) return; isSquashing = true;
            gsap.timeline({ onComplete: () => { isSquashing = false; motorActive = true; startMotorAnimation(); } })
                .to(busModel.scale, { x: 1.18, y: 0.65, z: 1.18, duration: 0.15 })
                .to(busModel.scale, { x: 0.92, y: 1.15, z: 0.92, duration: 0.15 })
                .to(busModel.scale, { x: 1, y: 1, z: 1, duration: 0.4, ease: 'elastic.out' });
        }
        ...
        function triggerRoadBump() {
            ...
            bumpTimeline.to(busModel.position, { y: -0.32, duration: 0.15 }).to(busModel.rotation, { z: 0.06, duration: 0.15 }, 0)
                .to(busModel.position, { y: -0.5, duration: 0.12 }).to(busModel.rotation, { z: -0.08, duration: 0.12 }, 0.15)
                .to(busModel.scale, { x: 1.08, y: 0.86, z: 1.08, duration: 0.08 }).to(busModel.rotation, { z: 0, duration: 0.15 })
                .to(busModel.scale, { x: 1, y: 1, z: 1, duration: 0.35, ease: 'elastic.out' });
        }

        function startMotorAnimation() {
            if (!busModel) return; isSquashing = true;
            gsap.timeline({ onComplete: () => isSquashing = false })
                .to(busModel.rotation, { z: 0.15, duration: 0.25 })
                .to(busModel.scale, { x: 1.25, y: 0.55, z: 1.25, duration: 0.25 }, 0)
                .to(busModel.rotation, { z: -0.05, duration: 0.2 })
                .to(busModel.scale, { x: 0.85, y: 1.28, z: 0.85, duration: 0.2 }, "-=0.2")
                .to(busModel.rotation, { z: 0, duration: 0.4, ease: 'elastic.out' })
                .to(busModel.scale, { x: 1, y: 1, z: 1, duration: 0.6, ease: 'elastic.out' }, "-=0.4");

            gsap.to({v:1}, { v:4.5, duration:1.5, onUpdate: function(){ particleSpeedMultiplier=this.targets()[0].v; } });
            gsap.to({v:0}, { v:0.18, duration:1.8, onUpdate: function(){ wheelRotationSpeed=this.targets()[0].v; } });
            gsap.to(ecoUnderglow, { intensity: 4, duration: 1 });
        }
